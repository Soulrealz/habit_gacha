...
- Modify: `app.json` (`expo install expo-sqlite` registers its config plugin here automatically — expected, leave it)
...
These are runtime dependencies and belong in `dependencies`, which is where
`expo install` puts them by default — no save flag needed. Installing `expo-sqlite`
also registers its config plugin in `app.json`; that edit is expected, and Task 4
depends on it, so leave it in place.
...
Use `--save-dev`, not the `--dev` that Expo's docs show. Under npm 10, `--dev` is an
install _filter_ (an alias for `--include=dev`), not a save location, so it silently
puts these three packages in `dependencies` instead of `devDependencies`.

```bash
npx expo install jest-expo jest @types/jest "--" --save-dev
```

Afterwards, confirm all three landed in `devDependencies`:

```bash
node -e "console.log(Object.keys(require('./package.json').devDependencies))"
```

Expected: the list includes `jest`, `jest-expo`, and `@types/jest`. If they appear in
`dependencies` instead, move them by hand and re-run `npm install` so the lockfile
records `dev: true`.
...
It spreads `GACHA_CONFIG` and overrides **only the rates and the pity threshold**. `dailyTicketCap` deliberately stays at its production value of 5: the habits vertical's device walkthrough needs the cap to actually bind in a dev build, and spec §6 authorises a rate override only. The spread keeps that boundary self-evident, so a future edit cannot silently let the dev cap diverge.
